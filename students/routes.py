...
from web_school.students.forms import AddStudentForm, UpdateInfoForm
...
def show_all_students():
    form = AddStudentForm()
    choices = [kls.name for kls in Studentgroup.query.all()]
    form.kls.choices = choices
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            new_student = Student(first_name=first_name, last_name=last_name,
                                  year=year, kls=kls, img=picture_file)
...
@students.route('/students/remove/', methods=['POST'])
def remove_student():
    if request.method == 'POST':
...
def student_profile(id):
    if request.method == 'GET':
        ...
        return render_template('student_profile.html', id=id, first_name=first_name,
                               last_name=last_name, year=year, kls=kls, img=img)
...
def edit_student_info(id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        return redirect(url_for('students.student_profile', id=id))
